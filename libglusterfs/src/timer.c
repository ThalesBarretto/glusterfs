...
gf_timer_proc(void *data)
{
    ...
    list_for_each_entry_safe(event, tmp, &reg->active, list)
    {
         ...
         * nobody -- a process-exit leak by design, not a bug to fix here.
         * Owners that care cancel during their fini, while the registry
         * is still alive, and get a truthful 0 back.
         */
        GF_FREE(event);
    }
