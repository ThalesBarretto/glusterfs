...
# do so unconditionally, even for a snapshot that a "snapshot clone" (or
# "restore") depends on; on a backend that refuses to remove such a snapshot
# (ZFS) the failure was swallowed and the backend snapshot orphaned.
#
...
#
# LVM twin of issue-4691-zfs.t (the selection is provider-agnostic); the
# backend orphan check itself needs the ZFS twin.
