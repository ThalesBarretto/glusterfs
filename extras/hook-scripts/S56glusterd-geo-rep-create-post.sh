...
if [ -f "${pub_file}" ]; then
    ...
    if [ "${secondary_user}" != "root" ]; then
        ...
        scp -P "${SSH_PORT}" "${SSH_OPT}" "${pub_file}" "${secondary_user}"@"${secondary_ip}":"${secondary_user}_home_dir/common_secret.pem.pub_tmp"
        ssh -p "${SSH_PORT}" "${SSH_OPT}" "${secondary_user}"@"${secondary_ip}" "mv ${secondary_user}_home_dir/common_secret.pem.pub_tmp ${secondary_user}_home_dir/${primaryvol}_${secondaryvol}_common_secret.pem.pub"
    else
        if [[ -z "${GR_SSH_IDENTITY_KEY}" ]]; then
            scp -P "${SSH_PORT}" "${SSH_OPT}" "${pub_file}" "${secondary_ip}":"${pub_file}_tmp"
            ssh -p "${SSH_PORT}" "${SSH_OPT}" "${secondary_ip}" "mv ${pub_file}_tmp ${pub_file_dname}/${primaryvol}_${secondaryvol}_${pub_file_bname}"
            ...
            scp -P "${SSH_PORT}" -i "${GR_SSH_IDENTITY_KEY}" "${SSH_OPT}" "${pub_file}" "${secondary_ip}":"${pub_file}_tmp"
            ssh -p "${SSH_PORT}" -i "${GR_SSH_IDENTITY_KEY}" "${SSH_OPT}" "${secondary_ip}" "mv ${pub_file}_tmp ${pub_file_dname}/${primaryvol}_${secondaryvol}_${pub_file_bname}"
